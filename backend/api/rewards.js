 ...
 *
 * POST /rewards/auth/setup      → set passcode (first time) [unused while the frontend gate is disabled]
 * POST /rewards/auth/verify     → verify passcode [unused while the frontend gate is disabled]
 *
...
// ─── Auth ─────────────────────────────────────────────────────────────────────
// Unused while the frontend passcode gate is disabled (see src/rewards/RewardsApp.jsx).
// Left in place so the gate can be re-enabled later without rebuilding this backend.
...
router.get("/categories", async (req, res) => {
  const s = req.neo4j.session();
  try {
    let result = await s.run(`MATCH (c:Category) RETURN c ORDER BY c.isCustom, c.name`);
    if (!result.records.length) {
      for (const cat of DEFAULT_CATEGORIES) {
        await s.run(
          `MERGE (c:Category {id: $id})
      ...
      result = await s.run(`MATCH (c:Category) RETURN c ORDER BY c.isCustom, c.name`);
    }
